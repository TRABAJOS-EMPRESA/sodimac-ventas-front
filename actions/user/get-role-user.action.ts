...
export async function getRoleUser(
  ...
  try {
    const response = await fetch(endpoint, {
      method: "POST",
      body: JSON.stringify({ email: email, ssoToken: ssoToken }),
      headers: {
...
//   GetInfoUserResp,
// } from "@/interfaces/user/get-role/get-info-user-resp.interface";
// import { decode } from "jsonwebtoken"; // Instala este paquete si aún no lo tienes.
...
//     const response = await fetch(endpoint, {
//       method: "POST",
//       body: JSON.stringify({ email: email, accessToken: accessToken, refreshToken: refreshToken }),
//       headers: {
//         "Content-Type": "application/json",
//       },
//     });
